...
class JadwalPelayanan extends CI_Controller {
    ...
    public function index(){
        $data['view'] = 'pasien/homecare/jadwal_pelayanan';
        ...
        $data['js_addons'] = '
                        ...
                        ';
        $data['list_jadwal_pelayanan'] = $this->db->query('
            SELECT 
                ...
                (SELECT GROUP_CONCAT(mph.nama SEPARATOR ",") FROM detail_data_registrasi_homecare ddrh INNER JOIN master_pelayanan_homecare mph ON ddrh.id_pelayanan = mph.id WHERE ddrh.id_registrasi = drh.id) as list_nama_pelayanan 
                    FROM jadwal_layanan_homecare jlh 
                        INNER JOIN data_registrasi_homecare drh ON drh.id = jlh.id_registrasi 
                        INNER JOIN master_user d ON d.id = drh.id_dokter 
                            WHERE drh.id_pasien = ? 
                                    ORDER BY drh.tanggal_pelayanan_pasien, drh.waktu_mulai', $this->session->userdata('id_user'))->result();
