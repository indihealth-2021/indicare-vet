...
class Register extends CI_Controller {
	...
	public function register(){
		if(!$this->session->has_userdata('id_user')){
			...
			$data = [
				...
				'password'=>password_hash($this->input->post('password'), PASSWORD_ARGON2ID,  ['memory_cost' => 2048, 'time_cost' => 4, 'threads' => 3]),
	...
	public function verifikasi(){
		...
		$user = $this->db->query('SELECT id,register_token,aktif FROM master_user WHERE id = '.$id_user)->row();
		if(!$this->session->has_userdata('id_user')){
			if($user){
				$user = $this->db->query('SELECT id,register_token,aktif FROM master_user WHERE id = '.$id_user.' AND register_token = "'.$token.'"')->row();
				if($user){
					if($user->aktif == 1){
